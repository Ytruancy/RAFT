        ...
        if not random:
            if cluster_feature:
                ...
                for val_id in range(len(train_dataset)):
                    ...
                    hog_features= hog(np.transpose(flow,(1,2,0)), orientations=9, \
                                      pixels_per_cell=(16,16),cells_per_block=(2,2), visualize=False, channel_axis=-1,feature_vector=True)
                    print("reduced map shape is {}".format(reduced_map.shape))
                    predictions.append(reduced_map)
